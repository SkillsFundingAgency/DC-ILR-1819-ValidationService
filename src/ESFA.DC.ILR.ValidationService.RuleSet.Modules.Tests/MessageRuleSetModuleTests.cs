...
namespace ESFA.DC.ILR.ValidationService.RuleSet.Modules.Tests
{
    public class MessageRuleSetModuleTests
    {
        ...
        public IReadOnlyCollection<Type> GetAssemblyRuleSet()
        {
            ...
            return asm.GetTypes()
                .Where(x => typeof(IRule<IMessage>).IsAssignableFrom(x) && !x.IsAbstract)
                .ToList();
        }
        ...
        public IReadOnlyCollection<IRule<IMessage>> GetContainerRuleSet()
        {
            ...
            return container.Resolve<IEnumerable<IRule<IMessage>>>().ToList();
        }
